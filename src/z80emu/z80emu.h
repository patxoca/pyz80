 ...
 * reason other than emulating the requested number of cycles. See
 * z80config.h.
 */
...
extern int Z80Interrupt (Z80_STATE *state, int data_on_bus, void *context);
...
/* Execute instructions as long as the number of elapsed cycles is smaller
 * than number_cycles, and return the number of cycles emulated. The emulator
 * can be set to stop early on some conditions (see z80config.h). The user
 * macros (see z80user.h) also control the emulation.
 */

extern int Z80Emulate (Z80_STATE *state, int number_cycles, void *context);
